      ...
      <div ref={box} style={{ position: "relative" }} onMouseMove={move} onMouseLeave={() => setHover(null)}>
        <svg viewBox={`0 0 ${W} ${H}`} width="100%" role="img" aria-label="Visitas y consultas en el tiempo">
          {[0, 0.25, 0.5, 0.75, 1].map((t) => (
            <g key={t}>
              <line x1={PL} x2={W - PR} y1={y(top * t)} y2={y(top * t)} stroke="#e8e5ee" strokeWidth="1" />
              <text x={PL - 8} y={y(top * t) + 4} textAnchor="end" fontSize="11" fill="#6b6478">
                {Math.round(top * t)}
              </text>
            </g>
          ))}
          ...
          {data.map((d, i) =>
            i % Math.ceil(data.length / 7) === 0 ? (
              <text key={i} x={x(i)} y={H - 8} textAnchor="middle" fontSize="11" fill="#6b6478">{d.label}</text>
            ) : null,
          )}

          <path d={path("visits")} fill="none" stroke={S1} strokeWidth="2" strokeLinejoin="round" strokeLinecap="round" />
          <path d={path("responses")} fill="none" stroke={S2} strokeWidth="2" strokeLinejoin="round" strokeLinecap="round" />

          {hover != null && (
            <>
              <line x1={x(hover)} x2={x(hover)} y1={PT} y2={H - PB} stroke="#c9c4d4" strokeWidth="1" />
        ...
        {h && (
          <div
            style={{
              ...
              background: "#fff", border: "1px solid #e8e5ee", borderRadius: 8,
              padding: ".55rem .75rem", fontSize: ".82rem", pointerEvents: "none",
              boxShadow: "0 4px 14px rgba(28,18,48,.1)", whiteSpace: "nowrap",
            }}
          >
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: ".8rem" }}>
      {rows.map((r) => (
        <div key={r.label}>
          ...
          <div style={{ height: 8, background: "#f1eff5", borderRadius: 4 }}>
