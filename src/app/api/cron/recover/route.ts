...
export async function GET(req: Request) {
  ...
  const pendientes = await sql<
    { id: string; email: string; full_name: string | null; firm: string; slug: string; accent: string;
      notify_email: string | null; funnel: string; funnel_slug: string | null; workflow_slug: string | null; lang: Lang }[]
  >`
  ...
  for (const p of pendientes) {
    const mail = recoveryEmail({
      ...
      area: p.funnel,
      url: linkParaRetomar({
        firmSlug: p.slug, funnelSlug: p.funnel_slug, workflowSlug: p.workflow_slug, sessionId: p.id,
      }),
      lang: p.lang,
    });
